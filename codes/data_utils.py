...
from mapper.rgraph import FOLRGraphMapper
from mapper.hgraph import FOLHGraphMapper
from mapper.graph import FOLGraphMapper
...
class build_FOL_graph:
    def __init__(self):
        ...
        self.stopwords_list = []
        with open('/home/majun/document/LTN_merge_no_fuzzy/datasets/stopwords.txt') as f:
            lines = f.readlines()
            for line in lines:
    ...
    def graph2matrix(self,graph):
        ad_matrix = nx.to_numpy_array(graph,weight='type')
...
class ABSADataset(Dataset):
    def __init__(self, fname, tokenizer,opt,tname):
        ...
        # self.fol_mapper = FOLRGraphMapper(tokenizer.tokenizer, '/home/majun/document/LTN_merge_no_fuzzy/datasets/stopwords.txt', '/home/majun/document/LTN_merge_ab/parent_graph.yaml', match, opt.nodes_num, opt.edge_num, opt.parent_need, opt.child2parent_need)
        
        # self.graph_mapper = FOLGraphMapper(tokenizer.tokenizer, '/home/majun/document/LTN_merge_no_fuzzy/datasets/stopwords.txt', '/home/majun/document/LTN_merge_ab/parent_graph.yaml', match, opt.nodes_num, opt.edge_num, opt.parent_need, opt.child2parent_need)
        
        self.hgraph_fol_mapper = FOLHGraphMapper(tokenizer.tokenizer, '/home/majun/document/LTN_merge/datasets/stopwords.txt', '/home/majun/document/LTN_merge/parent_graph.yaml', match, opt.nodes_num, opt.edge_num, opt.parent_need, opt.child2parent_need)
        self.semantic_similarity_threshold = opt.semantic_similarity_threshold
        ...
        def deal_data(sign_fname):
            # data_cache_filename = f"/home/majun/document/LTN_merge_hypergraph/cache/{self.model_name}/{'/'.join(sign_fname.split('/')[-2:]).replace('.csv','.pkl')}"
            # print(data_cache_filename)
            # if os.path.exists(data_cache_filename):
            #     print('loading data:', data_cache_filename)
            #     return pickle.load(open(data_cache_filename, 'rb')) 
            sall_data = []
            cnt = 1
            with open(sign_fname,'r',encoding='utf-8') as f:
                lines = csv.DictReader(f)
                i = 0
                for index,line in enumerate(tqdm(lines, 'dataset', 20000)):
                    # i += 1
                    # if i > 4:
                    #     break
                    # print(i)
                    ...
                    # # print(edge_index_gpt.shape)
                    # # print(edge_type_gpt.shape)
                    # # print(word_nodes_gpt.shape)
                    ...
                    # print(line[self.llm_name])
                    # print(i)
                    # try:
                    #     # generate_graph_data
                        
                    #     edge_index_gpt,edge_type_gpt,word_nodes_gpt = self.fol_mapper.generate_graph_data(line['Ans1'])
                    #     # assert torch.all(edge_type_gpt < 4)
                    #     # assert edge_index_gpt.max().item() <= word_nodes_gpt.unique().size(0)
                    #     edge_index_claude,edge_type_claude,word_nodes_claude = self.fol_mapper.generate_graph_data(line['Ans2'])
                    #     # assert torch.all(edge_type_claude < 4)
                    #     # assert edge_index_claude.max().item() <= word_nodes_claude.unique().size(0)
                    #     edge_index_lla,edge_type_lla,word_nodes_lla = self.fol_mapper.generate_graph_data(line['Ans3'])
                    #     # assert torch.all(edge_type_lla < 4)
                    #     # assert edge_index_lla.max().item() <= word_nodes_lla.unique().size(0)
                        
                    #     # _,ad_matrix_gpt = self.graph_mapper.generate_graph_data(line['Ans1'])
                    #     # assert torch.all(edge_type_gpt < 4)
                    #     # assert edge_index_gpt.max().item() <= word_nodes_gpt.unique().size(0)
                    #     # _,ad_matrix_claude = self.graph_mapper.generate_graph_data(line['Ans2'])
                    #     # assert torch.all(edge_type_claude < 4)
                    #     # assert edge_index_claude.max().item() <= word_nodes_claude.unique().size(0)
                    #     # _,ad_matrix_lla = self.graph_mapper.generate_graph_data(line['Ans3'])
                    #     # assert torch.all(edge_type_lla < 4)
                    #     # assert edge_index_lla.max().item() <= word_nodes_lla.unique().size(0)
                    # except:
                    #     i += 1
                    ...
                    hedge_index_gpt,hword_nodes_gpt = self.hgraph_fol_mapper.generate_hgraph_data(line['Ans1'])
                    hedge_index_claude,hword_nodes_claude = self.hgraph_fol_mapper.generate_hgraph_data(line['Ans2'])
                    hedge_index_lla,hword_nodes_lla = self.hgraph_fol_mapper.generate_hgraph_data(line['Ans3'])
                    hedge_index_fusion, hword_nodes_fusion = self.hgraph_fol_mapper.generate_hgraph_data(f'( {line["Ans1"]} ) ∧ ( {line["Ans2"]} ) ∧ ( {line["Ans3"]} )', semantic_similarity_threshold=self.semantic_similarity_threshold)
                    # hedge_index_gpt,hword_nodes_gpt = self.hgraph_fol_mapper.generate_hgraph_data(line[self.llm_name])
                    # edge_index_gpt,edge_type_gpt,word_nodes_gpt = Rgraph_PreModel(BG.run_build(line['FOL']))
                    # edge_index_claude,edge_type_claude,word_nodes_claude = Rgraph_PreModel(BG.run_build(line['FOL']))
                    # edge_index_lla,edge_type_lla,word_nodes_lla = Rgraph_PreModel(BG.run_build(line['FOL']))
                    ...
                    if 'bart' in opt.model_name or 'bert' in opt.model_name:
                        ...
                        bert_tt = [text, target, tokenizer.tokenizer.mask_token]
                        bert_tt = tokenizer.text_to_sequence(bert_tt,add=120,mmodel='base')
                        ...
                        if 'bert' in opt.pretrained_model_name and 'roberta' not in opt.pretrained_model_name:
                            data = {
                                ...
                                'concat_bert_indices': bert_tt[0], 
                                'concat_segments_indices': bert_tt[1],
                                ...
                                # 'graph_type': torch.tensor([edge_type_gpt.tolist(),edge_type_claude.tolist(),edge_type_lla.tolist()]).float(),
                                # 'graph_text': torch.tensor([word_nodes_gpt.tolist(),word_nodes_claude.tolist(),word_nodes_lla.tolist()]).long(), 
                                
                                # 'graph_index': torch.tensor([edge_index_gpt.tolist(),]).long(),
                                # 'graph_type': torch.tensor([edge_type_gpt.tolist()]).float(),
                                # 'graph_text': torch.tensor([word_nodes_gpt.tolist()]).long(), 
                                
                                # 'hgraph_index': torch.tensor([hedge_index_gpt.tolist(),hedge_index_claude.tolist(),hedge_index_lla.tolist()]).long(),
                                # 'hgraph_text': torch.tensor([hword_nodes_gpt.tolist(),hword_nodes_claude.tolist(),hword_nodes_lla.tolist()]).long(), 
                                'hgraph_index': torch.tensor([hedge_index_gpt.tolist(),hedge_index_claude.tolist(),hedge_index_lla.tolist(), hedge_index_fusion.tolist()]).long(),
                                'hgraph_text': torch.tensor([hword_nodes_gpt.tolist(),hword_nodes_claude.tolist(),hword_nodes_lla.tolist(), hword_nodes_fusion.tolist()]).long(), 
                                # 'hgraph_index': torch.tensor([hedge_index_gpt.tolist()]).long(),
                                # 'hgraph_text': torch.tensor([hword_nodes_gpt.tolist()]).long(),
            ...
            print(i)
            # data_cache_dirname = os.path.dirname(data_cache_filename)
            # if not os.path.exists(data_cache_dirname):
            #     os.makedirs(data_cache_dirname, exist_ok=True)
            # with open(data_cache_filename, 'wb') as f:
            #     pickle.dump(sall_data, f)
            return sall_data
        ...
        self.data = all_data
        # self.fol_mapper.save_cache()
        self.hgraph_fol_mapper.save_cache()
