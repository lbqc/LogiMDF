...
from cluster.predict import predict_cls_name
import os
import pickle


class FOLGraphMapper(object):
    ...
    def generate_graph_data(self, fol, parent_need=True, child2parent_need=False):
        ...
        # for node in child2parent_mapping.values():
            ...
            #     self.n_hop_cache[node] = nx.single_source_dijkstra(self.parent_graph, node, cutoff=1)[0]
    ...
    def _format_rgraph_data(self, data, features, max_num_nodes=None, max_num_edges=None):
        ...
        edge_index = np.asarray([edge[:2] for edge in edges], dtype=np.int64).transpose()
        adj_matrix = np.zeros((max_num_nodes, max_num_nodes), dtype=int)
        adj_matrix[edge_index[0], edge_index[1]] = 1
        
        return np.asarray(nodes), np.asarray(adj_matrix)
