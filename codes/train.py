...
from models.bert_fol_text_hgraph import BertFolHGraph
import pickle
...
class Instructor:
    def __init__(self, opt):
        ...
        # self.recall_scores = []
        self.filename_key = opt.model_name
        
        self.opt = opt
        LLM_name = BASEPATH.split('/')[-1]
        if '-' in self.opt.dataset and self.opt.dataset[-1] != '-':
            log_path = f'../if_log/{LLM_name}/{self.opt.model_name}_{self.opt.with_text}/cross/{opt.llm_name}_{self.opt.dataset[:2]}_{self.opt.dataset[3:]}/'
        elif self.opt.dataset[-1] == '-':
            log_path = f'../if_log/{LLM_name}/{self.opt.model_name}_{self.opt.with_text}/zero/{opt.llm_name}_{self.opt.dataset[:-1]}/'
        else:
            log_path = f'../if_log/{LLM_name}/{self.opt.model_name}_{self.opt.with_text}/in-domain/{opt.llm_name}/'
        if not os.path.exists(log_path):
            os.makedirs(log_path, exist_ok=True)
        logger = get_logger(f'{log_path}{self.opt.dataset}_{self.opt.model_name}_{self.opt.lid}_{self.opt.train_scale}.log',name='normal')
        self.logger = logger
        best_logger = get_logger(f'{log_path}best_{self.opt.dataset}_{self.opt.model_name}_{self.opt.lid}_{self.opt.train_scale}.log',name='best')
        ...
        # if os.path.exists(f'../cache/{self.opt.dataset}_{self.opt.parent_need}_train.pkl'):
        #     with open(f'../cache/{self.opt.dataset}_{self.opt.parent_need}_train.pkl', 'rb') as f:
        #         self.trainset = pickle.load(f)
        # else:
            
        #     self.trainset = ABSADataset(opt.dataset_file['train'], tokenizer,opt,'train')
        #     with open(f'../cache/{self.opt.dataset}_{self.opt.parent_need}_train.pkl', 'wb') as f:
        #         pickle.dump(self.trainset, f)
        # if os.path.exists(f'../cache/{self.opt.dataset}_{self.opt.parent_need}_test.pkl'):
        #     with open(f'../cache/{self.opt.dataset}_{self.opt.parent_need}_test.pkl', 'rb') as f:
        #         self.testset = pickle.load(f)
        # else:
            
        #     self.testset = ABSADataset(opt.dataset_file['test'], tokenizer,opt,'test')
        #     with open(f'../cache/{self.opt.dataset}_{self.opt.parent_need}_test.pkl', 'wb') as f:
        #         pickle.dump(self.testset, f)        
        # self.testset = ABSADataset(opt.dataset_file['test'], tokenizer,opt,'test')
        def load_or_create_dataset(file_path, dataset_file, split, do_cache=False):
            if os.path.exists(file_path):
                with open(file_path, 'rb') as f:
                    return pickle.load(f)
            else:
                dataset = ABSADataset(dataset_file, tokenizer, opt, split)
                if do_cache:
                    with open(file_path, 'wb') as f:
                        pickle.dump(dataset, f)
                return dataset
        
        cache_path = f'../cache/{self.opt.model_name}/'
        if not os.path.exists(cache_path):
            os.makedirs(cache_path, exist_ok=True)

        # Train dataset
        train_file_path = f'{cache_path}/{self.opt.dataset}_{self.opt.parent_need}_{self.opt.with_text}_train.pkl'
        self.trainset = load_or_create_dataset(train_file_path, opt.dataset_file['train'], 'train')

        # Test dataset
        test_file_path = f'{cache_path}/{self.opt.dataset}_{self.opt.parent_need}_{self.opt.with_text}_test.pkl'
        self.testset = load_or_create_dataset(test_file_path, opt.dataset_file['test'], 'test')
    ...
    def _train(self, criterion, optimizer, train_data_loader, val_data_loader,test_data_loader):
        ...
        for i_epoch in range(self.opt.num_epoch):
            ...
            for i_batch, batch in enumerate(train_data_loader):
                ...
                if self.opt.use_prompt == True and self.opt.model_name != "bert_spc":
                ...
                if t_batch % self.opt.log_step == 0:
                    ...
                    # nni.report_intermediate_result(round(maf1a*100,2))
                    # nni.report_intermediate_result(round(val_f1*100,2))
                    if val_f1 > max_maf1a:
                        ...
                        ma_precision, mi_precision, maf_precision, mif_precision, ma_recall, mi_recall, maf_recall, mif_recall, single_precision, single_recall = self._evaluate_acc_pr(test_data_loader)

                        best_test = [test_acc, test_f1,avg_f1,f_f1,a_f1,n_f1,maf1a,mif1a,i_epoch]
                        
                        best_test_pr = [ma_precision, mi_precision, maf_precision, mif_precision, ma_recall, mi_recall, maf_recall, mif_recall, single_precision, single_recall, i_epoch]
                        ...
                        self.logger.info(f'Test_pr: ma_precision: {round(best_test_pr[1]*100,2)}\tmi_precision: {round(best_test_pr[1]*100,2)}\tmaf_precision: {round(best_test_pr[2]*100,2)}\tmif_precision: {round(best_test_pr[3]*100,2)}\tma_recall: {round(best_test_pr[4]*100,2)}\tmi_recall: {round(best_test_pr[5]*100,2)}\tmaf_recall: {round(best_test_pr[6]*100,2)}\tmif_recall: {round(best_test_pr[7]*100,2)}\tsingle_precision: {best_test_pr[8]}\tsingle_recall: {best_test_pr[9]}')
                        # self.logger.info(f'Test: ma_precision: {best_test_pr[0]}\tmi_precision: {best_test_pr[1]}\tmaf_precision: {best_test_pr[2]}\tmif_precision: {best_test_pr[3]}\tma_recall: {best_test_pr[4]}\tmi_recall: {best_test_pr[5]}\tmaf_recall: {best_test_pr[6]}\tmif_recall: {best_test_pr[7]}\tsingle_precision: {best_test_pr[8]}\tsingle_recall: {best_test_pr[9]}\t')
        ...
        self.best_logger.info(f'Best_test: epoch:{best_test[-1]}\tma_precision: {round(best_test_pr[1]*100,2)}\tmi_precision: {round(best_test_pr[1]*100,2)}\tmaf_precision: {round(best_test_pr[2]*100,2)}\tmif_precision: {round(best_test_pr[3]*100,2)}\tma_recall: {round(best_test_pr[4]*100,2)}\tmi_recall: {round(best_test_pr[5]*100,2)}\tmaf_recall: {round(best_test_pr[6]*100,2)}\tmif_recall: {round(best_test_pr[7]*100,2)}\tsingle_precision: {best_test_pr[8]}\tsingle_recall: {best_test_pr[9]}')
        # nni.report_final_result(round(best_test[6]*100,2))
        return path
    ...
    def _evaluate_acc_pr(self, data_loader):
        t_targets_all, t_outputs_all = None, None
        self.model.eval()
        with torch.no_grad():
            for i_batch, t_batch in enumerate(data_loader):
                ...
                if t_targets_all is None:
                    ...
                    t_targets_all = torch.cat((t_targets_all, t_targets.cpu()), dim=0)
                    t_outputs_all = torch.cat((t_outputs_all, t_outputs.cpu()), dim=0)
        ma_precision = metrics.precision_score(t_targets_all, torch.argmax(t_outputs_all, -1), labels=[0,1], average='macro')
        mi_precision = metrics.precision_score(t_targets_all, torch.argmax(t_outputs_all, -1), labels=[0,1], average='micro')
        maf_precision = metrics.precision_score(t_targets_all, torch.argmax(t_outputs_all, -1), average='macro')
        mif_precision = metrics.precision_score(t_targets_all, torch.argmax(t_outputs_all, -1), average='micro')
        ma_recall = metrics.recall_score(t_targets_all, torch.argmax(t_outputs_all, -1), labels=[0,1], average='macro')
        mi_recall = metrics.recall_score(t_targets_all, torch.argmax(t_outputs_all, -1), labels=[0,1], average='micro')
        maf_recall = metrics.recall_score(t_targets_all, torch.argmax(t_outputs_all, -1), average='macro')
        mif_recall = metrics.recall_score(t_targets_all, torch.argmax(t_outputs_all, -1), average='micro')
        report = metrics.classification_report(t_targets_all, torch.argmax(t_outputs_all, -1))
        scores = report.split('\n\n')[1]
        scores = scores.split('\n')
        single_precision = [float(score.strip().split()[1]) for score in scores]
        single_recall = [float(score.strip().split()[2]) for score in scores]
        
        return ma_precision, mi_precision, maf_precision, mif_precision, ma_recall, mi_recall, maf_recall, mif_recall, single_precision, single_recall
    ...
    def run(self):
        ...
        train_scale_indices = torch.randperm(len(self.trainset))[:int(len(self.trainset) * self.opt.train_scale)]
        sampler = SubsetRandomSampler(train_scale_indices)
...
def main():
    ...
    parser.add_argument('--model_name', default='bert_fol_rgraph', type=str)
    ...
    parser.add_argument('--bert_lr', default=5e-6, type=float, help='try 5e-5, 2e-5 for BERT, 1e-3 for others')
    ...
    parser.add_argument('--nodes_num', default=50, type=int)
    parser.add_argument('--edge_num', default=400, type=int)
    parser.add_argument('--hyperedges_num', default=16, type=int)
    parser.add_argument('--gcn_num', default=4, type=int)
    parser.add_argument('--parent_need', default=False, type=bool)
    parser.add_argument('--child2parent_need', default=False, type=bool)
    parser.add_argument('--llm_name', default='w_cluster', type=str)
    parser.add_argument('--train_scale', default=1, type=float)
    parser.add_argument('--semantic_similarity_threshold', default=0.5, type=float)
    opt = parser.parse_args()
    
    # tuner_params = nni.get_next_parameter()
    # # os.environ['CUDA_VISIBLE_DEVICES'] = '5'
        
    # opt.dataset = tuner_params['dataset']
    # if tuner_params['dataset'] == "dt-hc":
    #     os.environ['CUDA_VISIBLE_DEVICES'] = '4'
    # elif tuner_params['dataset'] == "la-fm":
    #     os.environ['CUDA_VISIBLE_DEVICES'] = '6'
    # elif tuner_params['dataset'] == "fm-la":
    #     os.environ['CUDA_VISIBLE_DEVICES'] = '3'
    # elif tuner_params['dataset'] == "hc-dt":
    #     os.environ['CUDA_VISIBLE_DEVICES'] = '5'
    # else:
    #     os.environ['CUDA_VISIBLE_DEVICES'] = tuner_params['device']
    # # opt.device = tuner_params['device']
    # # opt.batch_size = tuner_params['batch_size']
    # opt.lr = tuner_params["lr"]
    # opt.nodes_num = tuner_params["nodes_num"]
    # opt.edge_num = tuner_params["edge_num"]
    # opt.gcn_num = tuner_params["gcn_num"]
    # opt.child2parent_need = True if tuner_params['child2parent_need'] != 0 else False
    # opt.parent_need = True if tuner_params['parent_need'] != 0 else False
    # opt.seed = tuner_params["seed"]
    # opt.dropout = tuner_params['dropout']
    ...
    input_colses = {
        ...
        'bert_fol_hgraph': ['bert_fol_inputs','bert_fol_type','bert_fol_mask','bert_text_inputs','bert_text_type','bert_text_mask','mlm_labels','hgraph_index', 'hgraph_text'], 
