...
class RGCNConv(MessagePassing):
    ...
    def forward(self, x: Union[OptTensor, Tuple[OptTensor, Tensor]],
        ...
        """
        # print('world')
        # assert edge_weight.shape == edge_type.shape
        # print('hello')
        ...
        if self.num_blocks is not None:  # Block-diagonal-decomposition =====
            ...
            for i in range(self.num_relations):
                ...
                h = self.propagate(tmp, x=x_l, edge_type_ptr=None, size=size, edge_weight=edge_weight[edge_type == i])
                ...
                for i in range(self.num_relations):
                    ...
                    if not torch.is_floating_point(x_r):
                        out = out + self.propagate(
                            ...
                            edge_type_ptr=None,
                            size=size,
                            edge_weight=edge_weight[edge_type == i],  # 在这里传入 edge_weight
                        )
                    else:
                        h = self.propagate(tmp, x=x_l, edge_type_ptr=None,
                                        size=size, edge_weight=edge_weight[edge_type == i])
                        out = out + (h @ weight[i])
    ...
    def message(self, x_j: Tensor, edge_type_ptr: OptTensor, edge_weight: OptTensor) -> Tensor:
        if (torch_geometric.typing.WITH_SEGMM and not is_compiling()
                and edge_type_ptr is not None):
            # Use `edge_weight` to weight the messages in segment_matmul operation
            return pyg_lib.ops.segment_matmul(x_j * edge_weight.unsqueeze(-1), edge_type_ptr, self.weight)
        
        # Weight the messages with `edge_weight` for the general case
